...
class pgConnector:
    ...
    def __init__(self, iface):
        ...
        #         queryDict[k]['linkStorage'] = city + queryDict[k]['tableExtantion'] + '.' + queryDict[k]['fileType']
        ...
        self.dlg.cc_pit_sign.clicked.connect(lambda: self.postgres_query)
    ...
    #         if 'buildings' in item:
    #             city = item[0:item.find('_')]
    #     self.dlg.label.setText(city)
    def postgres_query(self):
        city = None
        #self.dlg.label.setText('in progress')
        for item in QgsMapLayerRegistry.instance().mapLayers():
            if 'buildings' in item:
                city = item[0:item.find('_')]
        uiField = self.sender()
        ...
        # cur = conn.cursor()
        # #sql = """SELECT city_eng from public.links where city_eng;"""
        # sql = "UPDATE "+city+"."+city+"_cable_channel_pits SET microdistrict ="+city+"_microdistricts.micro_district FROM "+city+"."+city+"_microdistricts WHERE ST_Contains("+city+"."+city+"_microdistricts.coverage_geom, "+city+"."+city+"_cable_channel_pits.geom) ;UPDATE "+city+"."+city+"_cable_channel_pits SET district ="+city+"_microdistricts.district FROM "+city+"."+city+"_microdistricts WHERE ST_Contains("+city+"."+city+"_microdistricts.coverage_geom, "+city+"."+city+"_cable_channel_pits.geom) ;UPDATE "+city+"."+city+"_cable_channel_pits SET pit_district ="+city+"_coverage.notes FROM "+city+"."+city+"_coverage WHERE ST_Contains("+city+"."+city+"_coverage.geom_area, "+city+"."+city+"_cable_channel_pits.geom) and "+city+"."+city+"_coverage.geom_area is not null;"
        # #sql = "SELECT city, city_eng, region from public.links where city_eng='"+city+"';"
        # self.dlg.lineEdit.setText(sql)
        # query = cur.execute(sql)
        # conn.commit()

        # #self.dlg.label.setText('complete')
        # if cur.description!= None:
        #     result = cur.fetchall()
        #     self.dlg.lineEdit.setText('--'.join(''.join(elems) for elems in result))
        #     self.dlg.listWidget.clear()
        #     for row in result:
        #         self.dlg.listWidget.addItem('--'.join(''.join(items) for items in row))
        # else:
        #     cur.close()
        #     self.dlg.lineEdit.setText('None')
        #     self.dlg.listWidget.addItem('None')
        #cur.execute("SELECT city, city_eng, region from public.links ;")
