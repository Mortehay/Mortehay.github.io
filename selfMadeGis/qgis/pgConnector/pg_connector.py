...
class pgConnector:
    ...
    def __init__(self, iface):
        ...
        # air cables -----------------------------------------------------------------------------
        self.dlg.cableAirDataUpdate.clicked.connect(lambda: self.postgres_query('cableAirDataUpdate'))
    ...
    def postgres_query(self, button):
        ...
        def fileExists(fileLink, query):
            try:
                ret = urllib2.urlopen(fileLink)
                if ret.code == 200:
                    return query
            except:
                return False
        def cityExist(cities, city, query):
            try:
                for cityRow in cities:
                    if cityRow[1] == city:
                        return query.replace('cityGis',cityRow[0])
            except:
                return ''
        ...
        queryDict['cityBuildingDataUpdate']['queryList'] =[
            ...
            "CREATE TEMP TABLE temp AS SELECT cubic_hpname,  array_agg(cubic_house_id) as  agg_cubic_house_id, st_astext(ST_ConvexHull(ST_union(ST_makevalid(building_geom)))) as beauty_geom, sum(cubic_cnt::integer) as cubic_cnt, sum(cubic_cnt_docsis::integer) as cubic_cnt_docsis, sum(cubic_cnt_ktv::integer) as cubic_cnt_ktv, sum(cubic_cnt_atv::integer) as cubic_cnt_atv, sum(cubic_cnt_vbb::integer) as cubic_cnt_vbb, sum(cubic_cnt_eth::integer) as cubic_cnt_eth, sum(cubic_cnt_active_contr::integer) as cubic_cnt_active_contr from (select distinct on (cubic_house_id) * from "+city+"."+city+"_buildings ) as city where cubic_hpadress is not null group by cubic_hpname order by cubic_cnt desc; UPDATE "+city+"."+city+"_coverage SET cubic_cnt = temp.cubic_cnt, cubic_cnt_docsis = temp.cubic_cnt_docsis , cubic_cnt_ktv = temp.cubic_cnt_ktv , cubic_cnt_atv = temp.cubic_cnt_atv , cubic_cnt_vbb = temp.cubic_cnt_vbb, cubic_cnt_eth = temp.cubic_cnt_eth , cubic_cnt_active_contr = temp.cubic_cnt_active_contr FROM temp WHERE "+city+"_coverage.coverage_zone = temp.cubic_hpname and geom_area is not null;DROP TABLE temp;",
            fileExists(queryDict['cityBuildingDataUpdate']['linkHTTPStorage'],"CREATE TEMP TABLE temp(id serial, CITY character varying(100),REGION character varying(100), DISTR_NEW character varying(100), STREET character varying(100),HOUSE character varying(100), COMM text,CSD character varying(100),HOUSE_TYPE character varying(100),USO character varying(100), LNAME character varying(100),LADRESS character varying(100), HPNAME character varying(100),HPADRESS character varying(100), HPCODE character varying(100), FREQ character varying(100),DATE_BUILDING character varying(100), DATE_BUILDING_ETH character varying(100),DATE_CT character varying(100),SEGMENT character varying(100), DIGITAL_SEGMENT character varying(100), DIGITAL_STAGE character varying(100),DIGITAL_DATE character varying(100),SUBDEP character varying(100), BOX_TYPE character varying(100),HOUSE_ID character varying(100), SECTOR_CNT character varying(100),CNT character varying(100), PARNET text, SERV_PARNET text, NETTYPE character varying(100), CNT_ATV character varying(100),CNT_VBB character varying(100), CNT_ETH character varying(100),CNT_DOCSIS character varying(100), CNT_KTV character varying(100), CNT_ACTIVE_CONTR character varying(100),MAX_SPEED_ETHERNET character varying(100), MAX_SPEED_DOCSIS character varying(100), REPORT_DATE character varying(100)); select copy_for_testuser('temp(CITY,REGION,DISTR_NEW,STREET,HOUSE,COMM,CSD, HOUSE_TYPE,USO,LNAME,LADRESS,HPNAME,HPADRESS , HPCODE, FREQ,DATE_BUILDING, DATE_BUILDING_ETH, DATE_CT, SEGMENT, DIGITAL_SEGMENT,DIGITAL_STAGE,DIGITAL_DATE,SUBDEP,BOX_TYPE,HOUSE_ID, SECTOR_CNT, CNT,PARNET, SERV_PARNET,NETTYPE, CNT_ATV, CNT_VBB, CNT_ETH, CNT_DOCSIS, CNT_KTV,CNT_ACTIVE_CONTR, MAX_SPEED_ETHERNET, MAX_SPEED_DOCSIS, REPORT_DATE)', '"+queryDict['cityBuildingDataUpdate']['linkStorage'] +"', ',', 'utf-8') ; (SELECT CITY, STREET, HOUSE, HOUSE_ID, CNT, 'not in gis' as state  FROM temp WHERE HOUSE_ID  NOT IN(SELECT cubic_house_id FROM "+city+"."+city+"_buildings WHERE cubic_house_id IS NOT NULL) IS TRUE AND NETTYPE NOT IN('"+'Off_net SMART HD'.decode('utf-8')+"', '"+'Не подключен'.decode('utf-8')+"', '"+'Off_net SMART HD, 0к, РБ'.decode('utf-8')+"') ORDER BY CNT DESC) UNION (select cubic_city, cubic_street, cubic_house, cubic_house_id, cubic_cnt, 'not in cubic' as state from "+city+"."+city+"_buildings where cubic_house_id not in(select distinct house_id from temp) and cubic_network_type not in ('"+'Off_net SMART HD'.decode('utf-8')+"', '"+'Не подключен'.decode('utf-8')+"', '"+'Off_net SMART HD, 0к, РБ'.decode('utf-8')+"') ORDER BY cubic_cnt DESC);")

        ]
        #---cable channels channels part---------------
        queryDict['cableChannelChannelDataUpdate']['queryList'] =  [
            fileExists(queryDict['cableChannelChannelDataUpdate']['linkHTTPStorage'] ,"CREATE TEMP TABLE temp(id serial, pit_id_1 integer, pit_id_2 integer, distance varchar(100));select copy_for_testuser('temp( pit_id_1, pit_id_2, distance)', '"+queryDict['cableChannelChannelDataUpdate']['linkStorage'] +"', ';', 'windows-1251');INSERT INTO "+city+"."+city+"_cable_channels_channels( pit_id_1, pit_id_2, distance) SELECT pit_id_1, pit_id_2, distance FROM temp t WHERE not exists (SELECT 1 FROM "+city+"."+city+"_cable_channels_channels c where t.pit_id_1 = c.pit_id_1 and t.pit_id_2 = c.pit_id_2); "),
        ...
        queryDict['cableChannelPitsDataUpdate']['queryList'] = [
            "UPDATE "+city+"."+city+"_cable_channel_pits SET microdistrict ="+city+"_microdistricts.micro_district FROM "+city+"."+city+"_microdistricts WHERE ST_Contains("+city+"."+city+"_microdistricts.coverage_geom, "+city+"."+city+"_cable_channel_pits.geom) ;UPDATE "+city+"."+city+"_cable_channel_pits SET district ="+city+"_microdistricts.district FROM "+city+"."+city+"_microdistricts WHERE ST_Contains("+city+"."+city+"_microdistricts.coverage_geom, "+city+"."+city+"_cable_channel_pits.geom) ;UPDATE "+city+"."+city+"_cable_channel_pits SET pit_district ="+city+"_coverage.notes FROM "+city+"."+city+"_coverage WHERE ST_Contains("+city+"."+city+"_coverage.geom_area, "+city+"."+city+"_cable_channel_pits.geom) and "+city+"."+city+"_coverage.geom_area is not null;  update "+city+"."+city+"_cable_channel_pits set archive_link = 'http://"+final_list[0]+"/qgis-ck/tmp/archive/"+city+"/topology/pits/'||pit_id||'/';",
            "create temp table tmp as select pits.pit_id, roads.name from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_roads roads on(ST_DWithin(roads.geom,pits.geom,30))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.name from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp;create temp table tmp as select pits.pit_id, buildings.cubic_street from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_buildings buildings on(ST_DWithin(buildings.building_geom,pits.geom,20))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.cubic_street from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp;create temp table tmp as select pits.pit_id, buildings.cubic_street from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_buildings buildings on(ST_DWithin(buildings.building_geom,pits.geom,40))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.cubic_street from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp; create temp table tmp as select pits.pit_id, buildings.cubic_street from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_buildings buildings on(ST_DWithin(buildings.building_geom,pits.geom,60))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.cubic_street from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp; create temp table tmp as select pits.pit_id, buildings.cubic_street from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_buildings buildings on(ST_DWithin(buildings.building_geom,pits.geom,80))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.cubic_street from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp;create temp table tmp as select pits.pit_id, buildings.cubic_street from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_buildings buildings on(ST_DWithin(buildings.building_geom,pits.geom,100))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.cubic_street from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp; create temp table tmp as select pits.pit_id, roads.name from "+city+"."+city+"_cable_channel_pits pits right join "+city+"."+city+"_roads roads on(ST_DWithin(roads.geom,pits.geom,80))  where pits.street is null; update  "+city+"."+city+"_cable_channel_pits set street = tmp.name from tmp where tmp.pit_id = "+city+"_cable_channel_pits.pit_id; drop table tmp;"
        ]
        #-----ctv topology part-----------------------
        queryDict['ctvTopologyLoad']['queryList'] = [
            #"ALTER TABLE "+city+"."+city+"_ctv_topology DISABLE TRIGGER "+city+"_ctv_topology_air_cable_update;",
            fileExists(queryDict['ctvTopologyLoad']['linkHTTPStorage'],"CREATE TEMP TABLE temp( id serial, CITY character varying(100),STREET character varying(100),HOUSE character varying(100),FLAT character varying(100),CODE character varying(100),NAME character varying(100),PGS_ADDR character varying(100),OU_OP_ADDR character varying(100),DATE_REG character varying(100),COMENT character varying(100),UNAME character varying(100),NET_TYPE character varying(100),OU_CODE character varying(100),HOUSE_ID character varying(100), REPORT_DATE character varying(100), SECTOR character varying(100), FLOOR character varying(100), PIPE character varying(100)); select copy_for_testuser('temp(CITY, STREET, HOUSE ,FLAT ,CODE ,NAME ,PGS_ADDR ,OU_OP_ADDR ,DATE_REG ,COMENT ,UNAME ,NET_TYPE ,OU_CODE ,HOUSE_ID, REPORT_DATE, SECTOR, FLOOR, PIPE)', '"+queryDict['ctvTopologyLoad']['linkStorage'] +"', ',', 'utf-8') ;  INSERT INTO "+city+"."+city+"_ctv_topology(cubic_city, cubic_street, cubic_house, cubic_flat, cubic_code, cubic_name, cubic_pgs_addr, cubic_ou_op_addr, cubic_ou_code, cubic_date_reg, cubic_coment, cubic_uname, cubic_net_type, cubic_house_id) SELECT CITY,STREET,HOUSE,FLAT,CODE,NAME,PGS_ADDR,OU_OP_ADDR,OU_CODE,DATE_REG,COMENT,UNAME,NET_TYPE,HOUSE_ID FROM temp WHERE CODE NOT IN(SELECT cubic_code FROM "+city+"."+city+"_ctv_topology WHERE cubic_code IS NOT NULL);CREATE TEMP TABLE alien_cubic_code AS SELECT DISTINCT CODE FROM temp WHERE CODE IS NOT NULL ; DELETE FROM "+city+"."+city+"_ctv_topology WHERE cubic_code NOT IN(SELECT CODE FROM alien_cubic_code) ;"),
            #"ALTER TABLE "+city+"."+city+"_ctv_topology ENABLE TRIGGER "+city+"_ctv_topology_air_cable_update;"
        ]
        queryDict['ctvTopologyUpdate']['queryList'] = [
            #"ALTER TABLE "+city+"."+city+"_ctv_topology DISABLE TRIGGER "+city+"_ctv_topology_air_cable_update;",
            fileExists(queryDict['ctvTopologyUpdate']['linkHTTPStorage'],"CREATE TEMP TABLE temp( id serial, CITY character varying(100),STREET character varying(100),HOUSE character varying(100),FLAT character varying(100),CODE character varying(100),NAME character varying(100),PGS_ADDR character varying(100),OU_OP_ADDR character varying(100),DATE_REG character varying(100),COMENT character varying(100),UNAME character varying(100),NET_TYPE character varying(100),OU_CODE character varying(100),HOUSE_ID character varying(100), REPORT_DATE character varying(100), SECTOR character varying(100), FLOOR character varying(100), PIPE character varying(100)); select copy_for_testuser('temp( CITY, STREET, HOUSE ,FLAT ,CODE ,NAME ,PGS_ADDR ,OU_OP_ADDR ,DATE_REG ,COMENT ,UNAME ,NET_TYPE ,OU_CODE ,HOUSE_ID, REPORT_DATE, SECTOR, FLOOR, PIPE)', '"+queryDict['ctvTopologyUpdate']['linkStorage'] +"', ',', 'utf-8') ; UPDATE "+city+"."+city+"_ctv_topology SET cubic_city = temp.CITY, cubic_street = temp.STREET, cubic_house = temp.HOUSE, cubic_flat = temp.FLAT, cubic_code = temp.CODE, cubic_name = temp.NAME, cubic_pgs_addr = temp.PGS_ADDR, cubic_ou_op_addr = temp.OU_OP_ADDR, cubic_ou_code = temp.OU_CODE, cubic_date_reg = temp.DATE_REG, cubic_coment = temp.COMENT, cubic_uname = temp.UNAME, cubic_net_type = temp.NET_TYPE, cubic_house_id = temp.HOUSE_ID FROM  temp WHERE "+city+"."+city+"_ctv_topology.cubic_code = temp.CODE and "+city+"."+city+"_ctv_topology.cubic_house_id = temp.HOUSE_ID and "+city+"."+city+"_ctv_topology.cubic_name = temp.NAME;  (SELECT CITY,STREET,HOUSE,FLAT,CODE,NAME,PGS_ADDR,OU_OP_ADDR,OU_CODE,DATE_REG,COMENT,UNAME,NET_TYPE,HOUSE_ID , 'missing' as state FROM temp WHERE CODE NOT IN(SELECT cubic_code FROM "+city+"."+city+"_ctv_topology WHERE cubic_code IS NOT NULL))  UNION ALL (with data(CITY,STREET,HOUSE,FLAT,CODE,NAME,PGS_ADDR,OU_OP_ADDR,OU_CODE, DATE_REG,COMENT,UNAME,NET_TYPE,HOUSE_ID)  as (select CITY,STREET, HOUSE,FLAT, CODE,NAME,PGS_ADDR,OU_OP_ADDR,OU_CODE,DATE_REG,COMENT,UNAME,NET_TYPE,HOUSE_ID  from temp) select d.CITY, d.STREET, d.HOUSE, d.FLAT, d.CODE, d.NAME, d.PGS_ADDR, d.OU_OP_ADDR, d.OU_CODE, d.DATE_REG, d.COMENT, d.UNAME, d.NET_TYPE, d.HOUSE_ID, 'reused code' as rcode from data d where not exists (select 1 from "+city+"."+city+"_ctv_topology u where u.cubic_code = d.CODE and u.cubic_house_id = d.HOUSE_ID and u.cubic_name = d.NAME) and CODE IN(SELECT cubic_code FROM "+city+"."+city+"_ctv_topology WHERE cubic_code IS NOT NULL)) UNION ALL (with data(cubic_city, cubic_street, cubic_house, cubic_flat, cubic_code, cubic_name,cubic_pgs_addr, cubic_ou_op_addr, cubic_ou_code, cubic_date_reg, cubic_coment, cubic_uname, cubic_net_type, cubic_house_id)  as (select cubic_city, cubic_street, cubic_house, cubic_flat, cubic_code, cubic_name,cubic_pgs_addr, cubic_ou_op_addr, cubic_ou_code, cubic_date_reg, cubic_coment, cubic_uname, cubic_net_type, cubic_house_id  from "+city+"."+city+"_ctv_topology) select d.cubic_city, d.cubic_street, d.cubic_house, d.cubic_flat, d.cubic_code, d.cubic_name,d.cubic_pgs_addr, d.cubic_ou_op_addr, d.cubic_ou_code, d.cubic_date_reg, d.cubic_coment, d.cubic_uname, d.cubic_net_type, d.cubic_house_id, 'present state' as state from data d where not exists (select 1 from temp u where d.cubic_code = u.CODE and d.cubic_house_id = u.HOUSE_ID  and d.cubic_name = u.NAME) and cubic_code IN(SELECT CODE FROM temp WHERE CODE IS NOT NULL) );"),
            "UPDATE "+city+"."+city+"_ctv_topology SET equipment_geom = CASE WHEN cubic_name LIKE '"+'%Магистральный распределительный узел%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_firstpoint WHEN cubic_name LIKE '"+'%Магістральний оптичний вузол%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_thirdpoint WHEN cubic_name LIKE '"+'%Оптический узел%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_fourthpoint WHEN cubic_name LIKE '"+'%Оптичний приймач%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_firstpoint WHEN cubic_name LIKE '"+'%Блок питания%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_thirdpoint WHEN cubic_name LIKE '"+'%Передатчик оптический%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_secondpoint WHEN cubic_name LIKE '"+'%Порт ОК%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_secondpoint WHEN cubic_name LIKE '"+'%Домовой узел%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_thirdpoint WHEN cubic_name LIKE '"+'%Ответвитель магистральный%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_fourthpoint WHEN cubic_name LIKE '"+'%Распределительный стояк%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_secondpoint WHEN cubic_name LIKE '"+'%Магистральный узел%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_secondpoint WHEN cubic_name LIKE '"+'%Субмагистральный узел%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_thirdpoint WHEN cubic_name LIKE '"+'%Кросс-муфта%'.decode('utf-8')+"' THEN "+city+"."+city+"_buildings.building_geom_fourthpoint END FROM  "+city+"."+city+"_buildings WHERE "+city+"."+city+"_ctv_topology.equipment_geom IS NULL AND "+city+"."+city+"_ctv_topology.cubic_house_id = "+city+"."+city+"_buildings.cubic_house_id;",
        ...
        queryDict['etherTopologyUpdate']['queryList'] = [
            ...
            #do not work now"CREATE TEMP TABLE tmp_agr (cubic_switch_id varchar(100), cubic_parent_switch_id varchar(100), cubic_switch_role varchar(100), cubic_switch_agr_id varchar(100), level integer); INSERT INTO tmp_agr WITH RECURSIVE tmp_agr ( cubic_switch_id, cubic_parent_switch_id, cubic_switch_role, cubic_parent_switch_agr_id , LEVEL ) AS (SELECT T1.cubic_switch_id , T1.cubic_parent_switch_id , T1.cubic_switch_role , T1.cubic_parent_switch_id as cubic_parent_switch_agr_id , 1 FROM "+city+"."+city+"_switches T1 WHERE T1.cubic_parent_switch_role = 'agr' union select T2.cubic_switch_id, T2.cubic_parent_switch_id, T2.cubic_switch_role,tmp_agr.cubic_parent_switch_agr_id ,LEVEL + 1 FROM "+city+"."+city+"_switches T2 INNER JOIN tmp_agr ON( tmp_agr.cubic_switch_id = T2.cubic_parent_switch_id) ) select * from tmp_agr  ORDER BY cubic_parent_switch_agr_id; UPDATE "+city+"."+city+"_switches SET cubic_switch_agr_id = tmp_agr.cubic_switch_agr_id FROM tmp_agr WHERE "+city+"_switches.cubic_switch_id = tmp_agr.cubic_switch_id; UPDATE "+city+"."+city+"_switches SET cubic_switch_agr_id = null WHERE "+city+"_switches.cubic_switch_id not in (select distinct cubic_switch_id from tmp_agr where cubic_switch_id is not null);",
            fileExists(queryDict['etherTopologyUpdate']['linkHTTPStorageWorking'],"CREATE TEMP TABLE tmp( id serial, CITY varchar(100), ADDRESS  varchar(100), HOUSE_ID  varchar(100), SWITCH_ID varchar(100), street varchar(100), DOORWAY varchar(100), FLOOR varchar(100), MAC_ADDRESS varchar(100), IP_ADDRESS varchar(100),DEV_NAME  varchar(100), DEV_TYPE  varchar(100), SWITCH_MODEL  varchar(100), STATUS  varchar(100), DATE_CREATE  varchar(100), SERIAL_NUMBER varchar(100), DATE_CHANGE varchar(100), DEV_FULL_NAME text, MON_TYPE  varchar(100), REPORT_DATE  varchar(100)); select copy_for_testuser('tmp( CITY,ADDRESS,HOUSE_ID,SWITCH_ID,DOORWAY,FLOOR,MAC_ADDRESS,IP_ADDRESS,DEV_NAME,DEV_TYPE,SWITCH_MODEL,STATUS,DATE_CREATE,SERIAL_NUMBER,DATE_CHANGE,DEV_FULL_NAME,MON_TYPE,REPORT_DATE)', '"+queryDict['etherTopologyUpdate']['linkStorageWorking']+"', ',', 'utf-8') ; CREATE TEMP TABLE alien_cubic_switch_working_id AS SELECT DISTINCT SWITCH_ID FROM tmp WHERE SWITCH_ID IS NOT NULL ;DELETE FROM "+city+"."+city+"_switches_working WHERE switch_id NOT IN(SELECT SWITCH_ID FROM alien_cubic_switch_working_id) ; UPDATE "+city+"."+city+"_switches_working SET CITY = tmp.CITY, ADDRESS = tmp.ADDRESS, HOUSE_ID = tmp.HOUSE_ID, SWITCH_ID = tmp.SWITCH_ID, DOORWAY = tmp.DOORWAY,FLOOR = tmp.FLOOR, MAC_ADDRESS = tmp.MAC_ADDRESS, IP_ADDRESS = tmp.IP_ADDRESS, DEV_NAME = tmp.DEV_NAME, DEV_TYPE = tmp.DEV_TYPE, SWITCH_MODEL = tmp.SWITCH_MODEL, STATUS = tmp.STATUS, DATE_CREATE = tmp.DATE_CREATE, SERIAL_NUMBER = tmp.SERIAL_NUMBER, DATE_CHANGE = tmp.DATE_CHANGE, DEV_FULL_NAME = tmp.DEV_FULL_NAME, MON_TYPE = tmp.MON_TYPE, REPORT_DATE = tmp.REPORT_DATE FROM  tmp WHERE "+city+"."+city+"_switches_working.switch_id = tmp.SWITCH_ID;UPDATE "+city+"."+city+"_switches_working SET switches_geom = null  where switch_id in(select switches.switch_id from "+city+"."+city+"_switches_working switches  right join "+city+"."+city+"_buildings buildings on (switches.house_id= buildings.cubic_house_id) where ST_Contains(st_buffer(buildings.building_geom,1), switches.switches_geom) = false)  OR switch_id IN(select switches.switch_id from "+city+"."+city+"_switches_working switches right join "+city+"."+city+"_buildings buildings on(switches.house_id=buildings.cubic_house_id) right join "+city+"."+city+"_entrances entrances on (switches.house_id||'p'||switches.DOORWAY = entrances.cubic_entrance_id) where switches.switch_id is not null and st_equals(switches.switches_geom,entrances.geom) = false);select CITY, ADDRESS, HOUSE_ID, SWITCH_ID,DOORWAY, FLOOR,MAC_ADDRESS, IP_ADDRESS, DEV_NAME, DEV_TYPE,SWITCH_MODEL,STATUS,DATE_CREATE, SERIAL_NUMBER,DATE_CHANGE,DEV_FULL_NAME,MON_TYPE,REPORT_DATE from tmp where switch_id not in (select distinct switch_id from "+city+"."+city+"_switches_working where switch_id is not null);"),
            "UPDATE "+city+"."+city+"_switches_working SET switches_geom = CASE WHEN summ.geom IS NOT NULL THEN summ.geom WHEN summ.geom IS NULL THEN summ.building_geom_thirdpoint  END FROM (select switches.switch_id, switches.switches_geom, switches.house_id, switches.DOORWAY, buildings.building_geom_thirdpoint, entrances.cubic_entrance_id, entrances.geom, st_equals(switches.switches_geom,entrances.geom)  from "+city+"."+city+"_switches_working switches right join "+city+"."+city+"_buildings buildings on(switches.house_id=buildings.cubic_house_id) left join "+city+"."+city+"_entrances entrances on (switches.house_id||'p'||switches.DOORWAY = entrances.cubic_entrance_id) where switches.switch_id is not null) summ Where summ.switch_id = "+city+"."+city+"_switches_working.switch_id ;",
            "UPDATE "+city+"."+city+"_switches_working SET street = "+city+"_buildings.cubic_street, cubic_house_num = "+city+"_buildings.cubic_house from "+city+"."+city+"_buildings where "+city+"_buildings.cubic_house_id = "+city+"_switches_working.house_id;",
            cityExist(cities, city, "UPDATE "+city+"."+city+"_switches SET work_link ='http://work.volia.net/w2/eth/switch_info/act.current.php?ip='||cubic_ip_address||'&company=cityGis' WHERE cubic_ip_address IS NOT NULL;UPDATE "+city+"."+city+"_switches_working SET work_link ='http://work.volia.net/w2/eth/switch_info/act.current.php?ip='||ip_address||'&company=cityGis' WHERE ip_address IS NOT NULL;"),
            "UPDATE "+city+"."+city+"_switches_working SET switch_is_control = "+city+"_switches.cubic_switch_is_control, switch_is_opt82 = "+city+"_switches.cubic_switch_is_opt82, switch_contract_cnt = "+city+"_switches.cubic_switch_contract_cnt, cubic_switch_contract_active_cnt = "+city+"_switches.cubic_switch_contract_active_cnt, cubic_switch_location = "+city+"_switches.cubic_switch_location, switch_role = "+city+"_switches.cubic_switch_role  from "+city+"."+city+"_switches where "+city+"_switches_working.switch_id = "+city+"_switches.cubic_switch_id;"

        ]
        queryDict['cableAirDataUpdate']['queryList'] = [
            "UPDATE "+city+"."+city+"_cable_air set geom_start_point = null, cubic_code_start = null, cubic_name_start = null, cubic_coment_start = null, cubic_code_end = null, cubic_name_end = null, cubic_coment_end = null, geom_end_point =  null where geom_cable is null;",
            fileExists(queryDict['cableAirDataUpdate']['linkHTTPStorage'],"CREATE temp TABLE tmp (id serial,table_id varchar(100), cable_progect_link varchar(100), cable_mount_date varchar(100), cable_id varchar(100), cable_type varchar(100), cable_short_type_description varchar(100), cable_description varchar(100),  progect_number varchar(100), executive_doc_state varchar(100), cable_purpose varchar(100),  cubic_start_street varchar(100), cubic_start_house_num varchar(100), cubic_start_house_entrance_num varchar(100),link_fiber_welding_start varchar(100), cubic_end_street varchar(100), cubic_end_house_num varchar(100),  cubic_end_house_entrance_num varchar(100), link_fiber_welding_end varchar(100), total_cable_length varchar(100), notes2 varchar(100), rezerve1 varchar(100),  rezerve2 varchar(100),  rezerve3 varchar(100),  rezerve4 varchar(100), cubic_code_start varchar(100),  cubic_name_start varchar(100),  cubic_coment_start varchar(100),  cubic_code_end varchar(100),  cubic_name_end varchar(100),  cubic_coment_end varchar(100)); select copy_for_testuser('tmp (table_id , cable_progect_link , cable_mount_date ,  cable_id , cable_type , cable_short_type_description , cable_description ,  progect_number , executive_doc_state , cable_purpose , cubic_start_street , cubic_start_house_num,  cubic_start_house_entrance_num , link_fiber_welding_start , cubic_end_street , cubic_end_house_num ,  cubic_end_house_entrance_num , link_fiber_welding_end ,  total_cable_length , notes2 , rezerve1 , rezerve2 , rezerve3 , rezerve4 ,cubic_code_start,  cubic_name_start,  cubic_coment_start,  cubic_code_end,  cubic_name_end,  cubic_coment_end )', '"+queryDict['cableAirDataUpdate']['linkStorage']+"', ';', 'windows-1251') ; UPDATE "+city+"."+city+"_cable_air SET cable_progect_link  = null,cable_mount_date   = null  , cable_id   = null  , cable_type   = null  , cable_short_type_description   = null  , cable_description   = null, progect_number   = null , executive_doc_state   = null, cable_purpose  = null , cubic_start_house_id   = null  , cubic_start_street   = null  , cubic_start_house_num   = null , cubic_start_house_entrance_num   = null  , link_fiber_welding_start   = null  ,  cubic_end_house_id   = null  , cubic_end_street   = null  , cubic_end_house_num   = null , cubic_end_house_entrance_num   = null  , link_fiber_welding_end   = null  ,  total_cable_length   = null ,  notes2   = null , rezerve1   = null , rezerve2   = null , rezerve3   = null , rezerve4   = null, cubic_code_start = null, cubic_name_start = null, cubic_coment_start = null, cubic_code_end = null, cubic_name_end = null, cubic_coment_end = null from tmp where "+city+"_cable_air.table_id = tmp.table_id and    tmp.cable_mount_date  is null and tmp.cable_id  is null and tmp.cable_type is null and tmp.cable_short_type_description  is null and tmp.cable_description  is null and  tmp.progect_number  is null and tmp.executive_doc_state   is null and  tmp.cable_purpose  is null and  tmp.cubic_start_street  is null and tmp.cubic_start_house_num  is null and  tmp.cubic_start_house_entrance_num   is null and  tmp.cubic_end_street  is null and tmp.cubic_end_house_num   is null and  tmp.cubic_end_house_entrance_num   is null and  tmp.total_cable_length  is null and  tmp.notes2  is null and  tmp.rezerve1  is null and  tmp.rezerve2  is null and  tmp.rezerve3  is null and  tmp.rezerve4  is null and tmp.cubic_code_start is null and tmp.cubic_name_start is null and   tmp.cubic_coment_start is null and tmp.cubic_code_end is null and tmp.cubic_name_end is null and tmp.cubic_coment_end is null; UPDATE "+city+"."+city+"_cable_air SET  cable_progect_link  = 'https://"+final_list[0]+"/qgis-ck/csv/archive/"+city+"/air/'||tmp.progect_number||'/',cable_mount_date   = tmp.cable_mount_date  , cable_id   = tmp.cable_id  , cable_type   = tmp.cable_type  , cable_short_type_description   = tmp.cable_short_type_description  , cable_description   = tmp.cable_description  , progect_number   = tmp.progect_number  , executive_doc_state   = tmp.executive_doc_state  , cable_purpose  = tmp.cable_purpose , cubic_start_street   = tmp.cubic_start_street  , cubic_start_house_num   = tmp.cubic_start_house_num  , cubic_start_house_entrance_num   = tmp.cubic_start_house_entrance_num  , cubic_end_street   = tmp.cubic_end_street  , cubic_end_house_num   = tmp.cubic_end_house_num  , cubic_end_house_entrance_num   = tmp.cubic_end_house_entrance_num  ,  total_cable_length   = tmp.total_cable_length  ,  notes2   = tmp.notes2  , rezerve1   = tmp.rezerve1  , rezerve2   = tmp.rezerve2  , rezerve3   = tmp.rezerve3  , rezerve4   = tmp.rezerve4 FROM tmp WHERE "+city+"."+city+"_cable_air.table_id = tmp.table_id and tmp.table_id is not null and tmp.cable_type is not null and tmp.cable_short_type_description is not null and tmp.cable_description is not null;DROP TABLE tmp;"),
            "create temp table tmp as select start.table_id, start.cubic_code_start, start.geom_start_point,_end.cubic_code_end, _end.geom_end_point from (select air.table_id, case when st_distance(air.geom_start_point, ctv.equipment_geom) > 2.8 then air.cubic_code_start else null end as cubic_code_start, case when st_distance(air.geom_start_point, ctv.equipment_geom) > 2.8 then ctv.equipment_geom else null end as geom_start_point from "+city+"."+city+"_cable_air air, "+city+"."+city+"_ctv_topology ctv where air.cubic_code_start is not null and (air.cubic_code_start = ctv.cubic_code)) start join (select air.table_id, case when st_distance(air.geom_end_point, ctv.equipment_geom) > 2.8 then air.cubic_code_end else null end as cubic_code_end, case when st_distance(air.geom_end_point, ctv.equipment_geom) > 2.8 then ctv.equipment_geom else null end as geom_end_point  from "+city+"."+city+"_cable_air air, "+city+"."+city+"_ctv_topology ctv where air.cubic_code_end is not null and (air.cubic_code_end = ctv.cubic_code)) _end on(start.table_id = _end.table_id); update "+city+"."+city+"_cable_air_cable_geom set geom = st_setpoint(geom,0,tmp.geom_start_point) from tmp where tmp.table_id = "+city+"_cable_air_cable_geom.table_id and tmp.geom_start_point is not null; update "+city+"."+city+"_cable_air_cable_geom set geom = st_setpoint(geom, ST_NPoints(geom)-1,tmp.geom_end_point) from tmp where tmp.table_id = "+city+"_cable_air_cable_geom.table_id and tmp.geom_end_point is not null; drop table tmp;"
            ...
            if query:
                ...
                if cur.description!= None:
                    ...
                    if len(result) > 0:
                        ...
                        self.dlg.listWidget.addItem('----query with empty select----')
                elif cur.description is None:

                    #elf.dlg.listWidget.addItem('-query---'+query+'---query-')
                    self.dlg.listWidget.addItem('----query without select----')
            else:
                self.dlg.listWidget.addItem('----failed to load csv file----')
        ...
        #-----------------------------------------------
        #queryString = [x for x in queryDict[button]['queryList'] if x != False]

        self.dlg.lineEdit.setText('/////'.join([x for x in queryDict[button]['queryList'] if x != False]))

        #self.dlg.lineEdit.setText(queryDict['cableAirDataUpdate']['linkHTTPStorage'])
