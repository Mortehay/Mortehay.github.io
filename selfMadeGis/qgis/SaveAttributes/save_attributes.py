    ...
    def run(self):
        ...
        if result:
            ...
            with open(filename,'wb') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=fields, delimiter =';',doublequote=True,dialect='excel')
                writer.writeheader()
                for f in selectedLayer.getFeatures():
                    row ={}
                    for x in fields:
                        row[x] = replace_all(f[x],reps)
                    if len("".join(["%s" % (v) for k, v in row.items()])) !=0: writer.writerow(row) 
            
            self.iface.messageBar().pushMessage("INFO", "-----csv file was created-------", level=QgsMessageBar.INFO, duration=30)
