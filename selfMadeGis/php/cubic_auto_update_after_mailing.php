...
echo $query;
    ...
    $query = "create temp table eq_temp as (select cubic_name as equipment_name, 'ctv' as equipment_state, count(*) as equipment_count, CURRENT_DATE as update_time from ".$selectedCity.".".$selectedCity."_ctv_topology group by cubic_name) union (select cubic_switch_model as equipment_name, 'switches' as equipment_state, count(*), CURRENT_DATE as update_time from ".$selectedCity.".".$selectedCity."_switches group by cubic_switch_model, cubic_inventary_state) union (select name as equipment_name, 'hybrid' as equipment_state, count(*) as equipment_name, CURRENT_DATE as update_time from ".$selectedCity.".".$selectedCity."_hybrids group by name) union (select name as equipment_name, 'modems' as equipment_state, count(*) as equipment_name, CURRENT_DATE as update_time from ".$selectedCity.".".$selectedCity."_modems group by name); insert into ".$selectedCity.".".$selectedCity."_equipment_statistics(equipment_name, equipment_state, equipment_count, update_time) select  distinct on(equipment_name) * from eq_temp o_t where not exists(select 1 from ".$selectedCity.".".$selectedCity."_equipment_statistics i_n where o_t.equipment_name = i_n.equipment_name and o_t.equipment_state = i_n.equipment_state and o_t.update_time = i_n.update_time); delete from ".$selectedCity.".".$selectedCity."_equipment_statistics where equipment_name not in(select distinct equipment_name from eq_temp where equipment_name is not null); update ".$selectedCity.".".$selectedCity."_equipment_statistics set equipment_name = eq_temp.equipment_name, equipment_state = eq_temp.equipment_state, equipment_count = eq_temp.equipment_count, update_time = eq_temp.update_time from eq_temp where eq_temp.equipment_name = ".$selectedCity."_equipment_statistics.equipment_name;  ";
    echo $query.'<hr>';
