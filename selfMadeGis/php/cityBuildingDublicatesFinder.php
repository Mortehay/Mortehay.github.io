 	...
 	$selectedCity= $_POST['city_building_dublicates_finder_eng'];  
           ...
           $db = pg_connect( "$host $port $dbname $credentials"  );
            ...
            // echo "Opened database successfully\n";
                 ...
                 $sql = "SELECT openstreet_addr_street, openstreet_addr_housenumber, cubic_street, cubic_house, cubic_house_id FROM ".$selectedCity.".".$selectedCity."_buildings WHERE cubic_house_id IN (SELECT cubic_house_id FROM  (SELECT openstreet_addr_street, openstreet_addr_housenumber,cubic_house_id, count(*) AS count FROM ".$selectedCity.".".$selectedCity."_buildings  WHERE cubic_house_id IS NOT NULL GROUP BY 1, openstreet_addr_street, openstreet_addr_housenumber,cubic_house_id  ORDER BY count) count WHERE count >1) ORDER BY cubic_house_id, openstreet_addr_street, openstreet_addr_housenumber";
                  $ret = pg_query($db, $sql);
