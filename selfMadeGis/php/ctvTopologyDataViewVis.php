           ...
           $db = pg_connect( "$host $port $dbname $credentials"  );
            ...
            // echo "Opened database successfully\n";
                ...
                $arr_response = array(
                 ...
                 $description_sql = "SELECT cubic_city, cubic_street, cubic_house, cubic_name, cubic_code, cubic_ou_code, cubic_coment, cubic_pgs_addr   FROM ".$selectedCity.".".$selectedCity."_ctv_topology WHERE cubic_name NOT IN('Блок питания', 'Домовой узел', 'Магистральный узел', 'Ответвитель магистральный', 'Распределительный стояк', 'Порт ОК', 'Ответвитель домовой', 'Субмагистральный узел');";
                  ...
                  $links_sql = "SELECT cubic_code, cubic_ou_code, cubic_name,  cubic_pgs_addr   FROM ".$selectedCity.".".$selectedCity."_ctv_topology WHERE cubic_ou_code IN(SELECT  cubic_code FROM ".$selectedCity.".".$selectedCity."_ctv_topology WHERE cubic_code IS NOT NULL)  AND cubic_name NOT IN('Блок питания', 'Домовой узел', 'Магистральный узел', 'Ответвитель магистральный', 'Распределительный стояк', 'Порт ОК', 'Ответвитель домовой', 'Субмагистральный узел')".";"; 
                  ...
                  if($links_ret) {
                        ...
                        //-------------------------------------------------------
                        $mdods_sql = "SELECT DISTINCT cubic_city, cubic_street, cubic_house, cubic_name, cubic_code, cubic_ou_code, cubic_coment,  cubic_pgs_addr  FROM ".$selectedCity.".".$selectedCity."_ctv_topology WHERE cubic_name IN('Магистральный распределительный узел');";
                        $mdods_ret = pg_query($db, $mdods_sql);
                        
                        if($mdods_ret) {
                               while ($row = pg_fetch_row($mdods_ret))  {
                                ...
                                array_push($arr_response['mdods'], $mdods );
                              }
                        }
                  }
...
foreach ($sumObjectsArray as $sumObjectsArrayKey => $objectArray) {
  $arr = array(
    ...
    'label' => groupSelect($sumObjectsArray[$sumObjectsArrayKey]['cubic_name'])[4].' - '.$sumObjectsArray[$sumObjectsArrayKey]['cubic_street'].' , № '.$sumObjectsArray[$sumObjectsArrayKey]['cubic_house'],
...
foreach ($sumObjectsArray as $sumObjectsArrayKey => $objectArray) {
  $arr = array(
    ...
    'value' => groupSelect($sumObjectsArray[$sumObjectsArrayKey]['cubic_name'])[3],
    'color' => groupSelect($sumObjectsArray[$sumObjectsArrayKey]['cubic_name'])[1],
    'she' => $sumObjectsArray[$sumObjectsArrayKey]['cubic_pgs_addr']
  );
