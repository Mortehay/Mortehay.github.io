...
$query = "SELECT cubic_city, cubic_street, cubic_house_num, cubic_switch_role, cubic_switch_id, cubic_parent_switch_id, cubic_switch_role, cubic_inventary_state   FROM ".$selectedCity.".".$selectedCity."_switches;";
//echo $query;
$queryArrayKeys = array('cubic_city', 'cubic_street', 'cubic_house_num', 'cubic_switch_role', 'cubic_switch_id', 'cubic_parent_switch_id', 'cubic_switch_role', 'cubic_inventary_state');
...
$query = "SELECT cubic_switch_id, cubic_parent_switch_id, cubic_switch_role  FROM ".$selectedCity.".".$selectedCity."_switches WHERE cubic_parent_switch_id IN(SELECT  distinct cubic_switch_id FROM ".$selectedCity.".".$selectedCity."_switches WHERE cubic_switch_id IS NOT NULL);";
//echo $query;
$queryArrayKeys = array('cubic_switch_id', 'cubic_parent_switch_id', 'cubic_switch_role');
$retuenedArray = $newDBrequest -> dbConnect($query, $queryArrayKeys, true);
$sumObjectsArray = $retuenedArray;
//print_r($sumObjectsArray);
foreach ($sumObjectsArray as $sumObjectsArrayKey => $objectArray) {
